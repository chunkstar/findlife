...
if trips:
    ...
    selected_city = st.selectbox("Pick a trip to plan", options=[t["city"] for t in trips])
    if selected_city:
        ...
        # Beautiful interactive pie chart (Plotly - mobile friendly)
...
st.caption("Auto-refreshes hourly. Works great on your Pixel — just tap the three dots → 'Add to home screen' for app-like experience!")                        dt = datetime.fromisoformat(show["datetime"].replace("Z", "+00:00"))
                        v = show.get("venue", {})
                        all_shows.append({
                            "band": artist.title(),
                            "date": dt.date(),
